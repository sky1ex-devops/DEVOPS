...
По умолчанию эта команда ищет по файлам в рабочем каталоге. В качестве первого варианта вы можете использовать любой из параметров `-n` или `--line-number`, чтобы распечатать номера строк, в которых Git нашёл совпадения:
...
Вы также можете искать сложные комбинации строк, используя опцию `--and`, которая гарантирует, что будут отображены только строки, имеющие сразу несколько совпадений. Например, давайте поищем любые строки, которые определяют константу, имя которой содержит _любую_ из подстрок «LINK» или «BUF_MAX», особенно в более старой версии кодовой базы Git, представленной тегом v1.8.0 (мы добавим параметры `--break` и `--heading`, которые помогут вывести результаты в более читаемом виде):
